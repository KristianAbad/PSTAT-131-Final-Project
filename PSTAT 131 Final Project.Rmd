...
```{r}
x.music.train <- music.train %>%
  select(-song_popularity)
y.music.train <- music.train %>%
  select(song_popularity)
x.music.test <- music.test %>%
  select(-song_popularity)
y.music.test <- music.test %>%
  select(song_popularity)

# x <- data_4[,c(1,3:15)]
# y <- data_4[,"song_popularity"]
# x.music.train <- x[train,]
# y.music.train <- y[train,]
# x.music.test <- x[-train,]
# y.music.test <- y[-train,]
...
```{r}
set.seed(131)
bartfit <- gbart(x.music.train, y.music.train, x.test = x.music.test)
yhat.bart <- bartfit$yhat.test.mean
mean((y.music.test - yhat.bart)^2)
```
